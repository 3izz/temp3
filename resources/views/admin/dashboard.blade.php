...
<body>
    ...
    <div class="nav-right">
        ...
        <div class="profile-dropdown" data-section="profile">
            {{-- صورة الأدمن من المستخدم المسجل --}}
        ...
        <div class="container">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 30px;">
                    <div>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <a href="{{ route('admin.packages.index') }}" style="background-color: #4f46e5; color: white; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: 500;">
                                <i class="fas fa-box"></i> Manage Packages
                            </a>
                            <a href="{{ route('admin.packages.create') }}" style="background-color: #10b981; color: white; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: 500;">
                                <i class="fas fa-plus"></i> Create New Package
                            </a>
                            <a href="{{ route('courses.create') }}" style="background-color: #f59e0b; color: white; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: 500;">
                            ...
                            <table>
                                ...
                                <tbody>
                                    @forelse($courses->take(5) as $course)
                                        <tr>
                                            <td>{{ $course->title }}</td>
                                            <td><span class="status {{ $course->status }}">{{ ucfirst($course->status) }}</span></td>
            ...
            <section id="packages" class="content-section">
                        ...
                        <tbody>
                            ...
                            @forelse($packages as $package)
                                <tr>
                                    ...
                                    <td>{{ $package->courses->count() }} course(s)</td>
                                    <td>{{ ucfirst($package->target_type) }}</td>
                                    <td>
                                        <a href="{{ route('admin.packages.edit', $package) }}" class="btn btn-primary action-btn">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <form action="{{ route('admin.packages.destroy', $package) }}" method="POST" style="display: inline-block;" data-confirm="Are you sure you want to delete this package?">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" style="text-align: center; padding: 20px; color: #64748b;">No packages found. <a href="{{ route('admin.packages.create') }}">Create your first package</a></td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div style="margin-top: 20px;">
                    <a href="{{ route('admin.packages.index') }}" class="btn btn-primary">View All Packages</a>
                </div>
            </section>
    ...
    <div class="add-coach-form" style="max-width:500px; margin:auto; padding:20px; border:1px solid #ccc; border-radius:8px; background:#f9f9f9;">
        ...
        <form action="{{ route('admin.store-coach') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <!-- الاسم -->
            <div style="margin-bottom:15px;">
                <label>Name:</label>
                <input type="text" name="name" value="{{ old('name') }}" style="width:100%; padding:8px; border-radius:4px; border:1px solid #ccc;">
            ...
            <div style="margin-bottom:15px;">
                <label>Email:</label>
                <input type="email" name="email" value="{{ old('email') }}" style="width:100%; padding:8px; border-radius:4px; border:1px solid #ccc;">
            ...
            <div style="margin-bottom:15px;">
                ...
                @error('role') <div style="color:red; font-size:13px;">{{ $message }}</div> @enderror
            </div>
            ...
            <div style="margin-bottom:20px;">
                ...
                @error('profile_photo') <div style="color:red; font-size:13px;">{{ $message }}</div> @enderror
            </div>
