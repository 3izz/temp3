...
<div class="container" style="padding: 30px; max-width: 800px;">
    <div style="margin-bottom: 30px;">
        ...
        <h1 style="font-size: 2rem; color: #1f2937; margin-top: 10px;">Create New Package</h1>
    </div>
    ...
    <form action="{{ route('admin.packages.store') }}" method="POST" style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
        @csrf

        <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #374151;">Package Name *</label>
            <input type="text" name="name" value="{{ old('name') }}" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;" placeholder="e.g., College Program">
        </div>

        <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #374151;">Description</label>
            <textarea name="description" rows="3" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;" placeholder="Package description...">{{ old('description') }}</textarea>
        </div>

        <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #374151;">Price ($) *</label>
            <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;" placeholder="20000.00">
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="number" name="max_courses" value="{{ old('max_courses', 1) }}" min="1" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;" placeholder="e.g., 3">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <select name="target_type" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;">
                    <option value="group" {{ old('target_type') === 'group' ? 'selected' : '' }}>Group</option>
                    <option value="individual" {{ old('target_type') === 'individual' ? 'selected' : '' }}>Individual</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="delivery_mode" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;">
                    <option value="one_to_many" {{ old('delivery_mode') === 'one_to_many' ? 'selected' : '' }}>One to Many</option>
                    <option value="one_on_one" {{ old('delivery_mode') === 'one_on_one' ? 'selected' : '' }}>One on One</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="schedule_type" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; outline: none;">
                    <option value="fixed" {{ old('schedule_type') === 'fixed' ? 'selected' : '' }}>Fixed</option>
                    <option value="choose" {{ old('schedule_type') === 'choose' ? 'selected' : '' }}>Choose</option>
        ...
        <div style="margin-bottom: 20px;">
            <label style="display: flex; align-items: center; cursor: pointer;">
                <input type="checkbox" name="is_popular" value="1" {{ old('is_popular') ? 'checked' : '' }} style="margin-right: 8px;">
    ...
    <div style="border: 1px solid #cbd5e1; border-radius: 6px; padding: 15px; height: 300px; overflow-y: scroll;">
        @forelse($courses as $course)
            <label style="display: flex; align-items: center; padding: 10px; cursor: pointer; border-bottom: 1px solid #e2e8f0;">
                <input type="checkbox" name="course_ids[]" value="{{ $course->id }}" {{ in_array($course->id, old('course_ids', [])) ? 'checked' : '' }} style="margin-right: 10px;">
                <div>
